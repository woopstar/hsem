"""Number entity for HSEM EV target state-of-charge.

Defines :class:`HSEMEVTargetSocNumber`, a standard
:class:`homeassistant.components.number.NumberEntity` that lets users
adjust the EV target SoC (1-100 %) from the UI without re-running the
config/options flow.
...
class HSEMEVTargetSocNumber(NumberEntity, HSEMEntity):
    ...
    target SoC for an EV charging session at runtime.  The value is
    written back to ``config_entry.options`` so it persists across HA
    restarts.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            config_key: Config entry option key (e.g. ``hsem_ev_target_soc``).
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        Args:
            value: The new target SoC percentage.
        """
        clamped = max(
