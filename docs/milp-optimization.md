...
| $p_{\mathrm{exp}}[t]$ | Grid export price (currency/kWh). Before solving, `p_exp` is sanitised by clamping to `min(p_exp, p_imp)` to prevent an unbounded LP when `p_exp > p_imp` in any slot (issue #635). The legacy `min_export_price` clamp has been removed because the applier no longer physically blocks grid export below this price (issue #767). The export-revenue coefficient and terminal-SoC charge premium above actually use `p_exp_net[t] = p_exp[t] - export_fee_per_kwh` (issue #925, default fee `0.0`) — a fixed retailer margin/balancing-fee deduction so a positive-but-net-negative price is valued the same as a genuinely negative price. This netting is local to the objective; the battery-export floor mask (below) still uses the raw `p_exp`. |
