...
| **Attributes** | `listening` (bool — server socket bound), `port`, `requested_current_a` (whole amps in the last `SetChargingProfile` sent to the charger, `None` until the first profile has been sent — issue #886), `url` (best-effort `ws://<host>:<port>/` for the EVSE's OCPP config, when HA can resolve a reachable host) — present only when this EV's OCPP server is enabled; plus one entry per connected charger CPID with `status`, `power_w`, `transaction_id`, `connected_at` |
