...
| Export min price                  | `hsem_export_electricity_min_price`             | 0.0                                     | Minimum export price for intentional battery-to-grid discharge. By default (see the curtailment option below), the inverter does not throttle the grid feed-in limit for non-negative prices; surplus PV export is allowed up to `max_grid_export_power_kw` regardless of this threshold (issue #767). Negative export prices always trigger a physical block because exporting then costs money. |
| Export fee per kWh                | `hsem_export_fee_per_kwh`                       | 0.0                                     | Retailer margin/balancing fee per kWh exported (issue #925), subtracted from the raw export price everywhere export profitability is decided. A raw price that is positive but net-negative after this fee is treated like a negative price — PV export gets physically curtailed.                                                                                                                |
| Curtail PV below export min price | `hsem_curtail_pv_below_export_min_price`        | `False`                                 | Opt-in (issue #930). When enabled, the applier also physically blocks grid export (the same watt floor used for negative prices) whenever the export price drops below the minimum export price above, even if it is still non-negative. When disabled (default), the #767 behavior applies unchanged.                                                                                            |
...
| Field                       | Key                                                        | Default                             | Description                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------------------------- | ---------------------------------------------------------- | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| House consumption power     | `hsem_house_consumption_power`                             | `sensor.power_house_load`           | House load power sensor. **Watts expected** — automatically normalized from a declared `unit_of_measurement` (e.g. kW), and from live-timer samples too (issue #946)                                                                                                                                                                                                                                                 |
| Solar production power      | `hsem_solar_production_power`                              | `sensor.power_inverter_input_total` | PV production sensor. **Watts expected** — automatically normalized from a declared `unit_of_measurement` (e.g. kW), and from live-timer samples too (issue #946)                                                                                                                                                                                                                                                    |
| House includes EV           | `hsem_house_power_includes_ev_charger_power`               | `True`                              | Whether the raw live house meter includes EV charger power (CT position). HSEM separately derives per-EV normalized planner-baseline accounting from configured EV power telemetry                                                                                                                                                                                                                                   |
...
| Power meter phase A/B/C     | `hsem_huawei_solar_power_meter_phase_{a,b,c}_active_power` | —                                   | Optional. Live per-phase active-power sensors from the Huawei power meter. Required only when the phase-aware charging safety limiter below is enabled. **Watts expected** — automatically normalized from a declared `unit_of_measurement` (issue #946)                                                                                                                                                             |
...
| Allow Huawei discharge while charging | `hsem_ev_charger_force_max_discharge_power` | `False` | Permission (not a command): allows the Huawei house battery to discharge while this EV is charging. Huawei exposes one global discharge limit shared by the battery and every EV, so leaving this off forces battery discharge to 0 W whenever the EV is active/planned. Enabling it requires a ceiling above 0 W below (issue #991) |
| Max discharge power                   | `hsem_ev_charger_max_discharge_power`       | 0       | Discharge ceiling (W) while this EV charges, once permission above is granted. The applier's actual cap is the planner's own solved discharge rate for the slot, clamped to this ceiling — never a command on its own. Must be above 0 W when the permission is enabled; a 0 W ceiling silently blocks all discharge (issue #991)    |
...
| Field                   | Key                                                    | Default | Description                                                                                                                                                                                                                                          |
| ----------------------- | ------------------------------------------------------ | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Grid Import Energy      | `hsem_grid_import_energy_entity`                       | —       | Cumulative grid import meter. **kWh expected** — automatically normalized from a declared `unit_of_measurement` (e.g. Wh), including recorder-history reads used for ML training (issue #946). Also used as ML data source.                          |
| Grid Export Energy      | `hsem_grid_export_energy_entity`                       | —       | Cumulative grid export meter. **kWh expected** — automatically normalized from a declared `unit_of_measurement` (issue #946). Used for net consumption.                                                                                              |
| PV Energy               | `hsem_pv_energy_entity`                                | —       | Cumulative PV production meter. **kWh expected** — automatically normalized from a declared `unit_of_measurement` (issue #946).                                                                                                                      |
...
| Wind chill enabled      | `hsem_ml_consumption_wind_chill_enabled`               | `False` | Adds a wind-chill index (wind × heat loss below the reference temperature) to the ML model (issue #943). Wind history/forecast both derive from the weather forecast entity above — no separate wind sensor. Requires the temperature sensor above.  |
| Wind chill reference °C | `hsem_ml_consumption_wind_chill_reference_temperature` | 18.0    | Balance-point temperature (°C) above which wind no longer increases heat loss. Only used when wind chill is enabled. See "Choosing the reference temperature" in `docs/consumption-prediction.md` for how to pick and tune this value for your home. |
