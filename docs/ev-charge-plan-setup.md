...
| **EV Charger Phase Topology**                  | Yes        | `single_phase` | Safe default for single-phase or unknown wiring — every hard per-phase check assumes the whole command lands on one phase. Select `three_phase_balanced` only after confirming the charger draws balanced current across all three phases. Select `three_phase_switchable` for an auto-phase-switching charger (starts at 6 A on one phase, switches to balanced three-phase automatically) — its **Charger Min Power** is then the single-phase figure (e.g. `1380` W) and its **EV Charger Power** the three-phase nameplate (e.g. `11.0` kW). |
...
| ----------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Current EV SoC                | `hsem_ev_soc` sensor                         | Percentage (0–100 %). If the sensor is `unavailable`/`unknown`, HSEM refuses to plan EV charging until it reports again — an unknown SoC is never treated as 0 % (issue #988)                                                                                                                                                                                                                                                                                                                                       |
...
| Allow past target             | `hsem_ev_allow_charge_past_target_soc`       | Surplus charging past target, valued against export by avoided future import cost. The EV only absorbs PV the plan without it would not have stored in the house battery, and never stops the battery grid-charging (issues #775, #1015). Its charger ceiling follows the surplus down immediately — it is exempt from the ceiling deadband and from the slot-entry power hold. Governs **only** the at-or-above-target regime — below target, charging stays deadline-driven and may import from grid (issue #988) |
...
A charge-past-target EV is exempt (issue #1015): its ceiling already _is_ the PV
surplus, so holding a higher one would make up the difference from the grid.
...
**Phase switching (`three_phase_switchable`).** An auto-phase-switching
charger (go-e, Zaptec, Easee, Huawei SmartCharger in switchover mode, …)
starts a session at 6 A on one phase (1380 W) and switches to balanced
three-phase once the command exceeds what one phase can carry. HSEM models
both modes exactly: the planner's amp lattice contains the one-phase steps
...
would only mean 2 A per phase. For an auto-phase-switching charger
(`three_phase_switchable`), configure the single-phase minimum — 1380 W
becomes 6 A on one phase.
