...
| Allow past target             | `hsem_ev_allow_charge_past_target_soc`       | Surplus charging past target, valued against export by avoided future import cost. The house battery always takes its share of surplus PV first; the EV only absorbs what the battery cannot (issue #775). Governs **only** the at-or-above-target regime — below target, charging stays deadline-driven and may import from grid (issue #988) |
