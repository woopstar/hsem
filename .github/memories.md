...
| #24 | Harden safety and economic accounting | **Partially ported.** The `FinancialTracker` price-availability gating (issue #813) was extracted and ported natively: `import_electricity_price_available` / `export_electricity_price_available` on `LiveState`, sourced from actual sensor availability in `state_collector.py`, threaded through `accumulate_financials`, and gating `FinancialTracker.accumulate()`. The rest of the 2910-line diff (fork-monolithic coordinator, planner files) is fork-only architecture and not back-portable. |
