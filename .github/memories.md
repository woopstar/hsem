...
| #35 (`22dbc105`) | Add forecast reserve and stabilize phase limits | **Split.** Part 1 (planner-only battery export forecast reserve, `hsem_batteries_forecast_reserve_pct`) is self-contained and **ported** as issue #807 Stage 1 / PR #825. Parts 2–4 (`_active_primary_grid_charge_reference_w` / `_feedback_free_primary_phase_floor` in `phase_charge_limiter.py`; `_PrimaryGridChargeTransition` / `_SecondaryGridChargeTransition` / 45 s fail-closed deadline / `async_emergency_disable_grid_charge` in `working_mode_sensor.py`) are **N/A — re-verified 2026-08-25 (issue #831)**. This repo has no `phase_charge_limiter.py`, no `PhaseAwareChargeCommands`, no live per-phase (`grid_phase_power_w`) hardware-write-time limiter, and no `hsem_huawei_solar_batteries_grid_charge_maximum_power` entity at all — only the MILP *planning-time* aggregate/per-phase fuse constraint (`planner/milp/_phase_fuse.py`, EV-topology-only) exists. The entire feedback-loop-removal and transition/deadline logic is written *against* that missing runtime limiter and the PowMr `SECONDARY_MODE_CHARGE` lock from fork PRs #29–#32 (already N/A here per #792/#795/#798). Issue #807's own Stage 2 already reached this conclusion one day earlier (2026-08-24) and deferred it for the same reason; issue #831 was closed as a duplicate finding with no new code to port. Do not re-attempt without both an explicit PowMr-hardware request **and** a from-scratch live phase-limiter feature request — this is not a simple "finish the port" task. |

**Fork PR dependency graph (2026-08-25):** #26 → #27 → #28 form a chain ported in order (fork tags v7.2.0/v7.3.0/v7.3.1, tracked as #787/#788/#789, all merged). #29's self-contained core (#792) needs #26; its coordinator integration is now built on the `fix/797-...` branch (issue #797, COMMIT 3). #30 (#795) needs #26 + #29 and is N/A regardless. #31 (#797) is in active development on its branch — the previously-blocked coordinator prerequisite is resolved; remaining work is review/merge. #23/#24 portable parts are in PR #783; the two gaps found (#813, #814) are fixed (PRs #821, #820). #32 (#798) needs #26 + #29 + #30 for its full feature; its bug-fix-pattern audit half does not. #35's Part 1 is ported (#825/#807 Stage 1); its Parts 2-4 need the same missing runtime phase-limiter + PowMr transition machinery as #29-#32/#795/#798.
