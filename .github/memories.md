...
**Follow-up finding, the same day: the wire-level logging immediately surfaced a real spec-conformance bug.** Testing against a real go-e Charger V4 (firmware 60.6) with `hsem.ocpp_debug_start_charging`, the log showed `RemoteStartTransaction` and `SetChargingProfile` both `"Accepted"`, and the charger's own `StartTransaction` confirmed — yet the requested amperage was reportedly not actually enforced. Root cause: `OCPPCommandsMixin._send_set_charging_profile()` built its `TxDefaultProfile` with `chargingProfileKind: "Relative"`. Per OCPP 1.6 §3.11, `"Relative"` is only valid on a profile with purpose `"TxProfile"` (its schedule is anchored to that transaction's own start time) — `"TxDefaultProfile"` is deliberately transaction-agnostic (HSEM sends it immediately after `RemoteStartTransaction`, before it knows the transaction ID), so there is no transaction start for `"Relative"` to be anchored to. A charger's JSON-schema validation doesn't catch this cross-field business rule, so it can reply `"Accepted"` while silently never applying the semantically-undefined schedule — indistinguishable from a no-op without inspecting the raw wire payload, exactly what the new DEBUG logging exists to make visible. Fixed by sending `chargingProfileKind: "Absolute"` (with `startSchedule` omitted, meaning "effective immediately"), which is spec-valid for `TxDefaultProfile`.
