...
| #23 | Stabilize storage economics | **Not yet triaged in depth (quick pass 2026-08-24).** Mixed: touches fork-only `secondary_storage.py`/`future_value.py`/`secondary_storage_applier.py` and 272 lines of the fork's still-monolithic `coordinator.py` (PowMr-coupled, likely N/A), but also `cost_function.py`, `cost_types.py`, `engine_core.py`, `milp/_objective.py`, `milp/_diagnostics.py`, `utils/misc.py` — generic planner files that may carry a portable fix. Needs a real investigation pass before deciding. |
| #24 | Harden safety and economic accounting | **Not yet triaged in depth (quick pass 2026-08-24).** Very large (2910+/1036- across 74 files): `coordinator.py` (+1061), `financial_tracker.py` (+228 new), `prediction_tracker.py`, `working_mode_sensor.py`, and nearly every planner file. Title suggests real fixes, but the diff is entangled with the fork's monolithic coordinator and possibly fork-only `financial_tracker.py`. High effort to extract; needs a real investigation pass. |
| #26 | Model EV charger phase topology | **Already covered natively as issue #787 / PR #793** (`feat(ev): per-phase fuse constraints from charger phase topology`). File list matches almost exactly (`utils/phase_power.py`, `models/ev_config.py`, `flows/ev_planned_load_helpers.py`, `tests/planner/test_ev_charger_phase_topology.py`). No further action. |
| #27 | Publish the charging ceiling and re-portion stranded energy | **Already covered natively as PR #794** (`feat(ev): publish the charging ceiling and re-portion stranded energy` — identical title). No further action. |
| #28 | Make charging commands authoritative and executable | **Already covered natively as issue/PR #796** (`fix(ev): make EV charging commands authoritative and executable` — near-identical title, same file set: `ev_delivered_energy.py`, `integration_version.py`, `phase_power.py`, `milp/_write_results.py`, `engine_ev_milp.py`, `services.py`, `diagnostics.py`). No further action. |
| #29 | Stabilize current-slot live power | **Partially ported (issue #792).** Self-contained half: `utils/live_power.py` (`LivePowerEstimate`/`LivePowerWindow`, unwired), `PlannerInput.live_house_consumption_available` tri-state, `engine_population.py` honoring it (legacy `> 1e-9` fallback when `None`), and hardened `coordinator_builder._resolve_live_solar_measurement` / new `_resolve_live_house_measurement` (finite, non-negative, missing-entities-list check). **N/A / skipped:** the coordinator-level `LivePowerWindow` wiring (continuous sampling, replan gating, `LIVE_POWER_*` constants) is layered on the fork's still-monolithic `coordinator.py` plus its PowMr/secondary-storage subsystem (`secondary_site_power_delta_w`, `POWMR_*`) — `origin/main`'s coordinator has since been *split* into `coordinator.py` + `coordinator_{builder,cycle,data,helpers,lifecycle,planner_phase,state,tracking}.py`, and has no PowMr subsystem at all. Also skipped: `EVConfig.current_session_removed_from_base` and its `engine_ev_milp.py`/`milp_optimizer.py`/`_write_results.py`/`engine_core.py` consumers — this field doesn't exist anywhere in `origin/main`; it's fork-only bookkeeping from a prior (unrelated) fork commit, not something PR #29 itself introduces. |
| #30 (`623269a2`) | Safely release current-slot SBU lock | **N/A, investigated for issue #795.** Every hunk is cross-device Huawei/PowMr coordination: the new `SecondaryStorageConfig.current_slot_sbu_allow_utility_escape` flag and MILP relaxation only mean something against a `SECONDARY_MODE_SBU`/`UTILITY` lock this repo doesn't have; the coordinator's `acquire_secondary_utility_authority`/`secondary_utility_authority_is_valid` generation token exists solely to gate Huawei writes behind a verified *PowMr* Utility lock (`_verified_secondary_utility_lock_matches` reads `live.secondary_storage.soc_pct`/`load_power_w`); and the `working_mode_sensor.py` rework's whole purpose is sequencing "stop PowMr Utility, then allow Huawei" and recovering if PowMr authority is revoked mid-write — a two-inverter ordering problem that doesn't exist with one inverter. Unlike #29, there is no self-contained half: even the generic-looking `authority_is_valid: Callable[[], bool] \| None` parameter threaded into `inverter_verify.async_write_and_verify`/`applier.async_apply_battery_settings` would be dead code here, since no caller would ever supply a non-`None` value without a secondary device to revoke it. Do not re-attempt without an explicit PowMr-support request. **Re-verified 2026-08-24** directly against the live `623269a2` diff (not just this table's summary): confirmed no hunk is separable from the PowMr/SBU-lock authority handshake — same conclusion holds. PowMr support is not planned for this repo. |
| #31 | Make charging dispatch authoritative and solar-aware | **Not yet triaged in depth (quick pass 2026-08-24).** Does NOT appear to overlap with issue #801 (`live_power.py` house-availability work) — #31 is much larger (923+/632- across 24 files) and centers on `applier.py`, `phase_charge_limiter.py`, `working_mode_sensor.py`, `milp/_solver_execution.py`/`_write_results.py`/`_incumbent.py`/`_objective.py`. Looks like a genuinely un-ported EV feature. Needs a real investigation pass — best next candidate. |
| #32 | Throttle live charging current safely | **N/A — PowMr-scoped**, same pattern as #30. Adds `SecondaryStorageConfig.current_slot_charge_current_limit_a`, consumed only inside `_extend_secondary_constraints` gated on `SECONDARY_MODE_CHARGE` lock. No PowMr subsystem here, so no separable half. |
| #33 | Dependabot: bump python-dependencies group | **N/A** — this repo runs its own independent Dependabot (see PRs #799/#800 bumping ruff/pytest-socket separately); the fork's dependency set and versions aren't a "port" target. |
