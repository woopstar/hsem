...
## Live Phase-Aware Grid-Charge Safety Limiter (issue #831, Part 2 of 3)
...
**Canonical helper:** never re-derive the phase-headroom-minus-own-contribution math inline — always call `compute_phase_charge_limits()`. It floors the command to a 100 W step and fails closed to `0.0 primary_charge_power_w` (never `None`) whenever the live inputs make the command computation itself unsafe; `build_phase_aware_charge_commands()` additionally returns `primary_grid_charge_power_w=None` (no override, entity untouched) when the feature is disabled or the slot isn't a grid-charge slot — callers must distinguish `None` (leave alone) from `0.0` (write zero).
